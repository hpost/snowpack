...
export async function command(commandOptions: CommandOptions) {
  ...
  const bundlerPlugin = config.__bundler;
  ...
  for (const [dirDisk, dirDest, allFiles] of includeFileSets) {
    for (const locOnDisk of allFiles) {
      ...
      let builtFileOutput: SnowpackBuildMap;
      builtFileOutput = await buildFile(locOnDisk, fileContents, outLoc, {
        buildPipeline,
        messageBus,
      });
      allBuiltFromFiles.add(locOnDisk);
      const {baseExt, expandedExt} = getExt(outLoc);
      let contents = builtFileOutput[outLoc]?.contents;
      if (!contents) {
        continue;
      }
      ...
      switch (baseExt) {
        case '.js': {
          if (builtFileOutput[cssOutPath]) {
            await fs.mkdir(path.dirname(cssOutPath), {recursive: true});
            await fs.writeFile(cssOutPath, builtFileOutput[cssOutPath].contents, 'utf8');
  ...
  const installDest = path.join(buildDirectoryLoc, config.__webModulesDir);
